...
local apicompatibleutil = {
    ...
    enableHueBrtSatNoteColorSystem = function()
        if version < '0.7.0' then
    ...
    --- @return any property
    getPropertyFromClass = function(classVar, variable, allowMaps)
    ...
    --- @return string
    returnClientPrefName = function(clientPrefName)
    ...
    returnCompatibleClassName = function(className)
    ...
    returnCompatibleVariableName = function(className, variableName)
        assert(type(className) == 'string', 'Expected string for className, got ' .. type(className) .. '.') -- use only strings for className
    ...
    --- @param value any
    --- @param allowMaps? boolean
    setPropertyFromClass = function(classVar, variable, value, allowMaps)
        assert(type(classVar) == 'string', 'Expected string for classVar, got ' .. type(classVar) .. '.') -- use only strings for classVar
        assert(type(variable) == 'string', 'Expected string for variable, got ' .. type(variable) .. '.') -- use only strings for variable
        if not allowMaps then
            allowMaps = false
