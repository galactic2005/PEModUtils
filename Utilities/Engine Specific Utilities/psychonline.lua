...
function psychonline.getPlayerStatsTable(player)
    ...
    for index = 1, #psychonline.tablePlayerStatStrings do
        statToGrab = getPropertyFromClass('online.GameClient', 'room.state.player' .. playerType .. '.' .. psychonline.tablePlayerStatStrings[index])
...
function psychonline.toggleOpponentMode()
    if getPropertyFromClass('online.GameClient', 'room') ~= nil then
        return
    end
