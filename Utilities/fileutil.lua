local fileutil = {
    ...
    --- @return boolean
    isFolder = function(fileString, startFromCurrentModDirectory)
    ...
    --- @param path string
    --- @param startFromCurrentModDirectory boolean
    removeDepreciatesFromScript = function(path, startFromCurrentModDirectory)
    ...
    listFile = {
        ...
        --- @param startFromCurrentModDirectory boolean
        --- @return table
        read = function(path, startFromCurrentModDirectory)
            assert(type(path) == 'string', 'Expected string for path, got ' .. type(path) .. '.') -- use only strings for path
            assert(type(startFromCurrentModDirectory) == 'boolean', 'Expected boolean for startFromCurrentModDirectory, got ' .. type(startFromCurrentModDirectory) .. '.') -- use only booleans for startFromCurrentModDirectory
        ...
        --- @return string
        --- @return nil
        readLine = function(path, startFromCurrentModDirectory, linePosition)
            -- type asserts
            assert(type(path) == 'string', 'Expected string for path, got ' .. type(path) .. '.') -- use only strings for path
            assert(type(startFromCurrentModDirectory) == 'boolean', 'Expected boolean for startFromCurrentModDirectory, got ' .. type(startFromCurrentModDirectory) .. '.') -- use only booleans for startFromCurrentModDirectory
        ...
        ---@param startFromCurrentModDirectory boolean
        write = function(path, tableToInsert, startFromCurrentModDirectory)
            -- type asserts
            assert(type(path) == 'string', 'Expected string for path, got ' .. type(path) .. '.') -- use only strings for path
